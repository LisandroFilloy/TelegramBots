...
@bot.message_handler(func=(lambda x: True if x.lower() == 'comida' else False))
def food(message):
...
@bot.message_handler(func=(lambda x: True if x.lower() == 'suplemento' else False))
...
@bot.message_handler(func=(lambda x: True if x.lower() == 'deposicion' else False))
...
@bot.message_handler(func=(lambda x: True if x.lower().replace('í', 'i') in ['sintoma'] else False))
def sintoma(message):
    _today = datetime.datetime.today()
    _date = '{}-{}-{} {}:{}'.format(_today.year, _today.month, _today.day, _today.hour, _today.minute)
...
@bot.message_handler(func=(lambda x: True if x.lower() in ['comentario'] else False))
...
@bot.message_handler(func=(lambda x: True if x.lower() in ['id'] else False))
def id(message):
